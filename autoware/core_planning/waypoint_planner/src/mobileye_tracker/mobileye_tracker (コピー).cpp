...
private:
	...
	void createCarTrackingWaypoints(const autoware_msgs::Lane::ConstPtr &lane, const StopLineInfo &stop_line_info)
	{
		...
		//double distance = 0;
		int tracking = trackJudge(cmdacc, foreground_obs, new_lane.waypoints[0].twist.twist.linear.x);//追跡タイプを計算
		double temporary_fixed_velocity_kmh;
		...
		for(int waycou=0; waycou<new_lane.waypoints.size(); waycou++)
		{
			...
			if(waycou==stop_line_info.waypoint_index_)
			{
				...
				{
					temporary_fixed_velocity_kmh = 0;
					obs_index = waycou;
				}
				else if(tracking == TRACKING_DECELERATION)
				{
					temporary_fixed_velocity_kmh = foreground_obs.velocity_mps * 3.6;
					obs_index = waycou;
				}
		...
		//経路に停止線情報を設定
		if(tracking == TRACKING_DECELERATION || tracking == TRACKING_STOP || tracking == TRACKING_EMG_DECELERATION)
		{
			double distance = 0;
			...
			for(int waycou=obs_index-1; waycou>=0; waycou--)
			{
				...
				double cmd_pass_vel =  (cmdacc.L0 > cmdacc.Lt_prime) ? temporary_fixed_velocity_kmh : v3;
				std::string cmd_pass_vel_str = (cmdacc.L0 > cmdacc.Lt_prime) ? "temporary_fixed_velocity_kmh" : "v3";
				...
				if(distance > distance_th)
				{
					if(tracking == TRACKING_DECELERATION)
					{
						...
						curr_waypoint.waypoint_param.temporary_fixed_velocity = cmd_pass_vel;
					}
				...
				{
					curr_waypoint.twist.twist.linear.x = temporary_fixed_velocity_kmh / 3.6;
				}
			}
		}
