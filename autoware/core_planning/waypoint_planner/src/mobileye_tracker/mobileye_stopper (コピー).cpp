...
private:
	...
	const static double CAR_COUNTER_MAGN_ = 1;
	...
	//障害物探索
	/*void obstacleSearch(const autoware_msgs::TransformMobileyeObstacleList &obs_data_list, StopInfo &ret)
	{
		resetStopInfo(ret);

		std::vector<double> dt_list(obs_data_list.data.size(), DBL_MAX);
		std::vector<int> id_list(obs_data_list.data.size(), -1);
		...
		for(int i=0; i<waypoints_.waypoints.size(); i++)
		{
			...
			for(int obs_ind=0; obs_ind<obs_data_list.data.size(); obs_ind++)
			{
				const geometry_msgs::Pose &obs_pose = obs_data_list.data[obs_ind].pose;
		...
		for(int i=0; i<waypoints_.waypoints.size(); i++)
		{
			...
			for(int obs_ind=0; obs_ind<obs_data_list.data.size(); obs_ind++)
			{
				if(way_id == id_list[obs_ind])
				{
					...
					ret.distance_ = obs_data_list.data[obs_ind].orig_data.obstacle_pos_x;//dt_list[obs_ind];
					ret.pos_ = obs_data_list.data[obs_ind].pose.position;
					ret.vel_ = obs_data_list.data[obs_ind].orig_data.obstacle_rel_vel_x;
					ret.obs_id_ = obs_data_list.data[obs_ind].orig_data.obstacle_id;
					return;
				}
			}
		}
	...
	void obstacleSearch(const autoware_msgs::TransformMobileyeObstacleList &obs_data_list, StopInfo &ret)
	{
		...
		for(autoware_msgs::TransformMobileyeObstacle tmo : obs_data_list.data)
		{
			...
			if(dt < min_dt)
			{
				min_dt = dt;
				ret.way_id_ = -1;
				ret.distance_ = tmo.orig_data.obstacle_pos_x;//obs_data_list.data[obs_ind].orig_data.obstacle_pos_x;//dt_list[obs_ind];
				ret.pos_ = tmo.pose.position;//obs_data_list.data[obs_ind].pose.position;
				ret.vel_ = tmo.orig_data.obstacle_rel_vel_x * 3.6;//obs_data_list.data[obs_ind].orig_data.obstacle_rel_vel_x;
				ret.obs_id_ = tmo.orig_data.obstacle_id;//obs_data_list.data[obs_ind].orig_data.obstacle_id;
			}
		}
	}
	...
	{
		list.insert(list.begin(), vel);
		dt_list.insert(dt_list.begin(), dt);
		if(list.size() > AVE_VEL_MAX_)
		{
			list.resize(AVE_VEL_MAX_);
			dt_list.resize(AVE_VEL_MAX_);
		}
		double sum = 0;
		for(int i=0; i<list.size(); i++)
		{
			sum += list[i] / dt_list[i];
			std::cout << "aaaaaaaaaaaaa," << list[i] << "," << dt_list[i] << "," << list[i] / dt_list[i] << "," << sum << std::endl;
		}std::cout << std::endl;
		return sum / list.size();
	}

	StopInfo prev_stop_info_;
	ros::Time prev_obstacle_time_;
	std::vector<double> obstacle_vel_list_;
	std::vector<double> obstacle_dt_list_;
	void callbackMobileyeObstacle(const autoware_msgs::TransformMobileyeObstacleList::ConstPtr &msg)
	{
		...
		prev_stop_info_ = stop_info;
		prev_obstacle_time_ = msg->header.stamp;
	}
