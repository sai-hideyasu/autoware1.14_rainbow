...
private:
										   ...
										   double &stop_distance)
	{
		...
		for (int i = start_index; i < l.waypoints.size(); ++i)
		{
			...
			double v;// = prev_vel + sqrt(square_vel + 2 * deceleration * distance);
			if(prev_vel < 15 / 3.6)
			{
				v = prev_vel + sqrt(square_vel + 2 * deceleration * distance);
			}
			else
			{
				v = prev_vel + distance * 0.05;
			}
	...
	SelectedLine stop_line_search(autoware_msgs::Lane& way)
	{
		std_msgs::Int32 dis;
		dis.data = -1;

		SelectedLine selected = {false, false, false, false, -1, -1};
		for(int i=0; i<way.waypoints.size() || i<config_.search_distance; i++)
		{
			/*if(light_color_ == TRAFFIC_LIGHT_RED && way.waypoints[i].waypoint_param.signal_stop_line > 0)
			{
			...
			if(selected.on_temporary_ || selected.on_signal_ || selected.on_object_ || selected.on_oncoming_)
			{
				...
				if(selected.on_signal_ || selected.on_oncoming_) fixed_velocity_ = 0;
				else fixed_velocity_ = std::max(way.waypoints[i].waypoint_param.temporary_fixed_velocity / 3.6, 0.0);
													...
													int ahead_cnt, int behind_cnt)
	{
		...
		SelectedLine stop_index = stop_line_search(new_lane); //std::cout << stop_index.waypoint_index_ << std::endl;
		...
		else if(stop_index.on_object_ == true) limit_vel = std::max(10.0 / 3.6, stop_way.waypoint_param.temporary_fixed_velocity / 3.6);
		...
		new_lane = apply_deceleration(new_lane, dec, stop_line_to_baselink_index, behind_cnt + 1, fixed_velocity_, false, -1, stop_distance);
		...
		new_lane = apply_deceleration(new_lane, dec, reverse_stop_index, ahead_cnt + 1, fixed_velocity_, true, limit_vel, stop_distance);
		std::reverse(new_lane.waypoints.begin(), new_lane.waypoints.end());
