...
private:
	...
	ros::Subscriber sub_config_, sub_local_waypoints_, sub_no_curve_waypoints_, sub_avoidance_switch_, sub_base_waypoints_;
	ros::Timer timer_;
	...
	bool lock_;//これがfalseの場合は、経路変更を行わない
	...
	//global_waypointsの更新があった場合、変更経路の保持をリセット
	void callbackBaseWaypoints(const autoware_msgs::LaneArray &msg)
	{
		//avoidanceListClear();
		//avoidance_switch_ = AVOIDANCE_SWITCH_OFF;
	}
	...
	bool table_create_flag_;
	void createCorrespondenceTable()
	{
		if(table_create_flag_ == false)
		{
	...
	void waypointPlusClothoidWidth(const double clothoid_length, const bool avoidance_flag, const int start_index, const bool use_curve_waypoints)
	{
		...
		{
			emptyWaypointsInfoPublish(ros::Time::now()); return;
		}

		int sgn;// = (avoidance_flag == true) ? 1 : -1;
		if(avoidance_flag == true)
			sgn = (config_.avoid_kaind_flag == autoware_config_msgs::ConfigClothoidAvoidance::AVOID_KAIND_DETOUR) ? -1 : 1;
		else
			sgn = (config_.avoid_kaind_flag == autoware_config_msgs::ConfigClothoidAvoidance::AVOID_KAIND_DETOUR) ? 1 : -1;
		...
		if(avoidance_flag == true)
		{
			avoidance_list_.move_width =
				(config_.avoid_kaind_flag == autoware_config_msgs::ConfigClothoidAvoidance::AVOID_KAIND_DETOUR) ? -config_.move_lane_length : config_.move_lane_length;
	...
	void emptyWaypointsInfoPublish(ros::Time stamp)
	{
	...
	void avoidancePublisher(int start_index, bool use_curve_waypoints, ros::Time nowtime)
	{
		if(lock_)
		{
			...
			{
				if(table_clothoid_and_lane_[i].lane_length_ > config_.clothoid_length)//)ObstacleAvoidance::path_length)
				{
	...
	void returnPublisher(int start_index, bool use_curve_waypoints, ros::Time nowtime)
	{
		...
		for(int i=0;i<table_clothoid_and_lane_.size() ;i++)
		{
			if(table_clothoid_and_lane_[i].lane_length_ > config_.clothoid_length)//path_length)
			{
				clothoid_length = table_clothoid_and_lane_[i].clothoid_length_;
				break;
			}
	...
	{
		//global waypointsの読み込みが無い場合は経路変更をリセットし、lock_(経路変更禁止フラグ)をfalseにする
...
public:
		...
		, lock_(false)
	{
		...
		sub_base_waypoints_ = nh_.subscribe("/lane_waypoints_array", 1, &ObstacleAvoidance::callbackBaseWaypoints, this);
		timer_ = nh.createTimer(ros::Duration(0.1), &ObstacleAvoidance::callbackTimer, this);
	...
	void run(const bool use_curve_waypoints, int forced)
	{
		...
		{
			emptyWaypointsInfoPublish(nowtime);
		}
