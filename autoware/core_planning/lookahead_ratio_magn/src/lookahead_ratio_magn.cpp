...
public:
	...
	tf::Matrix3x3 math_angular(geometry_msgs::Pose pose, geometry_msgs::Pose cur_pose)
	{
		...
		//if((current_q.getX() == 0 && current_q.getY() == 0 && current_q.getZ() == 0) &&
		//	(way_q.getX() == 0 && way_q.getY() == 0 && way_q.getZ() == 0))
		return sa_m;
	}
	...
	void run()
	{
		...
		if(waypose_.size() < 2)
		{
			...
			dist.baselink_distance = 999;
			dist.baselink_angular = 181 * M_PI / 180.0;
			dist.front_baselink_distance = 999;
			dist.front_baselink_angular = 181 * M_PI / 180.0;
