...
private:
  ...
  void gnssDirectionCallback(const autoware_msgs::GnssDirection::ConstPtr &direc_msg);
  ...
  autoware_msgs::GnssDirection gnss_direction_;
};
...
WaypointSaver::WaypointSaver() : private_nh_("~")
{
  ...
  sub_gnss_direction_ = nh_.subscribe("gnss_direction", 10, &WaypointSaver::gnssDirectionCallback, this);
...
void WaypointSaver::gnssDirectionCallback(const autoware_msgs::GnssDirection::ConstPtr &direc_msg)
{
  gnss_direction_ = *direc_msg;
}
...
void WaypointSaver::outputProcessing(geometry_msgs::Pose current_pose, double velocity) const
{
  ...
  if (!receive_once)
  {
    ofs << "x,y,z,yaw,velocity,change_flag,gnss_latitude,gnss_longitude,gnss_height,gnss_azimuth" << std::endl;
    ofs << std::fixed << std::setprecision(4) << current_pose.position.x << "," << current_pose.position.y << ","
        ...
        << std::fixed << std::setprecision(6) <<gnss_direction_.nmea_latitude << "," << gnss_direction_.nmea_longitude << ","
        << gnss_direction_.nmea_height << "," << gnss_direction_.azimuth_rad << std::endl;
    receive_once = true;
  ...
  else
  {
    ...
    if (distance > interval_)
    {
      ofs << std::fixed << std::setprecision(4) << current_pose.position.x << "," << current_pose.position.y << ","
          ...
          << std::fixed << std::setprecision(6) <<gnss_direction_.nmea_latitude << "," << gnss_direction_.nmea_longitude << ","
          << gnss_direction_.nmea_height << "," << gnss_direction_.azimuth_rad * 180 / M_PI << std::endl;
