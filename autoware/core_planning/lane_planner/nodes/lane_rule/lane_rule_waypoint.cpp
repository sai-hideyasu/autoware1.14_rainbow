...
double config_velocity_limit = 25;
...
double front_bumper_to_baselink;//sai add 2021_07_16
                                       ...
                                       size_t fixed_cnt, double fixed_vel, bool reverse_flag, double velocity_limit)
{
  ...
  {
    if(i - start_index > 100) break;//local waypointsとの状況再現のため、local waypointsの個数と合わせる
    ...
    std::cout << l.waypoints[i].waypoint_param.id << "," << v << "," << square_vel << "," << acceleration << "," << distance << std::endl;
    ...
    else if(reverse_flag == false)//停止線より下側は中断処理をしない
      break;
  }
...
autoware_msgs::Lane apply_crossroad_acceleration(const autoware_msgs::Lane& lane, double acceleration)
{
  ...
  /*for (const size_t i : end_indexes)
  {
		...
		if(stop_line_to_baselink < front_bumper_to_baselink)
		{
			stop_line_to_baselink_index = i;
		}

    //l = apply_acceleration(l, acceleration, i, 1, l.waypoints[i].twist.twist.linear.x, false, config_velocity_limit / 3.6);
    l = apply_acceleration(l, acceleration, stop_line_to_baselink_index, 1, l.waypoints[stop_line_to_baselink_index].twist.twist.linear.x, false, config_velocity_limit / 3.6);
  }*/
  for(int i=0; i<l.waypoints.size(); i++)
  {
    if(l.waypoints[i].waypoint_param.signal_stop_line > 0)
    {
   		// front バンパーが一時停止線にあるときに、ベースリンクに一番近い2個のwaypointのうち前のもの。
      // ≒ stop_index - 5
      double stop_line_to_baselink = 0;
      ...
      for(int cou=i; cou>=1; cou--)
      {
        double x1 = l.waypoints[cou].pose.pose.position.x;
        double y1 = l.waypoints[cou].pose.pose.position.y;
        double x2 = l.waypoints[cou-1].pose.pose.position.x;
        double y2 = l.waypoints[cou-1].pose.pose.position.y;
        double sx = x1-x2,  sy = y1-y2;
        stop_line_to_baselink += sqrt(sx*sx + sy*sy);
        if(stop_line_to_baselink >= front_bumper_to_baselink)
        {
      ...
      l = apply_acceleration(l, acceleration, stop_line_to_baselink_index, 1, l.waypoints[i].twist.twist.linear.x, false, config_velocity_limit / 3.6);
    }
  }
  ...
  /*for (const size_t i : reverse_start_indexes)
  {
    // front バンパーが一時停止線にあるときに、ベースリンクに一番近い2個のwaypointのうち前のもの。
		...
		double stop_line_to_baselink = 0;
		int stop_line_to_baselink_index;
		for(int cou=i+1; cou<l.waypoints.size()-1; cou++)
		{
			double x1 = lane.waypoints[cou].pose.pose.position.x;
			double y1 = lane.waypoints[cou].pose.pose.position.y;
			double x2 = lane.waypoints[cou-1].pose.pose.position.x;
    ...
    //l = apply_acceleration(l, acceleration, i, 1, l.waypoints[i].twist.twist.linear.x, true, config_velocity_limit / 3.6);
    l = apply_acceleration(l, acceleration, stop_line_to_baselink_index, 1, l.waypoints[stop_line_to_baselink_index].twist.twist.linear.x, true, config_velocity_limit / 3.6);
  }*/
  for(int i=0; i<l.waypoints.size(); i++)
  {
    if(l.waypoints[i].waypoint_param.signal_stop_line > 0)
    {
      ...
      double stop_line_to_baselink = 0;
      int stop_line_to_baselink_index;
      for(int cou=i; cou<l.waypoints.size()-1; cou++)
      {
        double x1 = l.waypoints[cou].pose.pose.position.x;
        double y1 = l.waypoints[cou].pose.pose.position.y;
        double x2 = l.waypoints[cou-1].pose.pose.position.x;
                                                ...
                                                size_t behind_cnt)
{
  ...
  /*for (const size_t i : indexes)
  {
    ...
    //l = apply_acceleration(l, acceleration, i, behind_cnt + 1, 0, false, config_velocity_limit / 3.6);
    l = apply_acceleration(l, acceleration, stop_line_to_baselink_index, behind_cnt + 1, 0, false, config_velocity_limit / 3.6);
  }*/
  for(int i=0; i<l.waypoints.size(); i++)
  {
    if(l.waypoints[i].waypoint_param.signal_stop_line > 0)
    {
      ...
      l = apply_acceleration(l, acceleration, stop_line_to_baselink_index, 1, 0, false, config_velocity_limit / 3.6);
    }
  }
  std::reverse(l.waypoints.begin(), l.waypoints.end());
  ...
  /*for (const size_t i : reverse_indexes)
  {
    ...
    //l = apply_acceleration(l, acceleration, i, ahead_cnt + 1, 0, true, config_velocity_limit / 3.6);
    l = apply_acceleration(l, acceleration, stop_line_to_baselink_index, ahead_cnt + 1, 0, true, config_velocity_limit / 3.6);
  }*/
  for(int i=0; i<l.waypoints.size(); i++)
  {
    if(l.waypoints[i].waypoint_param.signal_stop_line > 0)
                                        ...
                                        double stopline_search_radius)
{
  ...
  //sai add
  std::vector<size_t> stop_indexes2;
  for(int i=0; i<stop_indexes.size(); i++)
  {
    int ind = stop_indexes[i];
    int corr = lane.waypoints[ind].waypoint_param.signal_stop_vector_correction;
    int ind2 = ind + corr;
    if(ind2 >= 0 && ind2 <= lane.waypoints.size() -1) stop_indexes2.push_back(ind2);
  }
  std::sort(stop_indexes2.begin(), stop_indexes2.end());
  return stop_indexes2;

  //std::sort(stop_indexes.begin(), stop_indexes.end());
  //return stop_indexes;
}

geometry_msgs::Pose waypoint_pose_correct(const geometry_msgs::Pose p, double corr)
{
  Eigen::Vector3d po(p.position.x, p.position.y, p.position.z);
  double roll, pitch, yaw;
	geometry_quat_to_rpy(roll, pitch, yaw, p.orientation);
                                                ...
                                                double stopline_search_radius, size_t ahead_cnt, size_t behind_cnt)
{
  ...
  for(int ind : indexes)
  {
    l.waypoints[ind].waypoint_param.signal_stop_line = 1;
    l.waypoints[ind].pose.pose = waypoint_pose_correct(l.waypoints[ind].pose.pose, l.waypoints[ind].waypoint_param.signal_stop_vector_position_correction);
  }
...
void config_parameter(const autoware_config_msgs::ConfigLaneRule& msg)
{
  ...
  config_velocity_limit = msg.velocity_limit;
