...
static pose initial_pose, predict_pose, predict_pose_imu, predict_pose_odom, predict_pose_imu_odom, previous_pose,
    ndt_pose, ndt_pose_corr, current_pose, current_pose_corr, current_pose_imu, current_pose_odom, current_pose_imu_odom, localizer_pose;
...
static Eigen::Matrix4f tf_btol, tf_btol_corr;
...
ros::Publisher pub_correct, pub_tmp;

static double yaw_correct_deg, position_y_correct;//向きと位置の補正

void correctPublish()
{
  autoware_msgs::LocalizerCorrect cor_msg;
  cor_msg.header.stamp = ros::Time::now();
  cor_msg.distance_correct = position_y_correct;
  cor_msg.angle_correct_deg = yaw_correct_deg;
  pub_correct.publish(cor_msg);
}

static void math_tf_btol_corr()
{
  Eigen::Translation3f tl_btol(_tf_x, _tf_y + position_y_correct, _tf_z);                 // tl: translation
  Eigen::AngleAxisf rot_x_btol(_tf_roll, Eigen::Vector3f::UnitX());  // rot: rotation
  Eigen::AngleAxisf rot_y_btol(_tf_pitch, Eigen::Vector3f::UnitY());
  Eigen::AngleAxisf rot_z_btol(_tf_yaw + yaw_correct_deg*M_PI/180.0, Eigen::Vector3f::UnitZ());
  tf_btol_corr = (tl_btol * rot_z_btol * rot_y_btol * rot_x_btol).matrix();
}

static void localizer_correct_callback(const autoware_msgs::LocalizerCorrect::ConstPtr &msg)
{
  ...
  {
    yaw_correct_deg = msg->angle_correct_deg;
  }
  ...
  {
    position_y_correct = msg->distance_correct;
  }
  math_tf_btol_corr();
  correctPublish();
}
...
static void way_param_callback(const autoware_msgs::WaypointParam::ConstPtr &msg)
{
  /*if(msg->ndt_yaw_correct_deg >= -10 && msg->ndt_yaw_correct_deg <= 10)
  {
    Eigen::Translation3f tl_btol(_tf_x, _tf_y, _tf_z);                 // tl: translation
    Eigen::AngleAxisf rot_x_btol(_tf_roll, Eigen::Vector3f::UnitX());  // rot: rotation
    Eigen::AngleAxisf rot_y_btol(_tf_pitch, Eigen::Vector3f::UnitY());
    Eigen::AngleAxisf rot_z_btol(_tf_yaw + msg->ndt_yaw_correct_deg*M_PI/180.0, Eigen::Vector3f::UnitZ());
    tf_btol_corr = (tl_btol * rot_z_btol * rot_y_btol * rot_x_btol).matrix();
  }*/

  if(msg->ndt_yaw_correct_deg >= -autoware_msgs::LocalizerCorrect::ANGLE_CORRECT_DEG_TH &&
    msg->ndt_yaw_correct_deg <= autoware_msgs::LocalizerCorrect::ANGLE_CORRECT_DEG_TH)
  {
    yaw_correct_deg = msg->ndt_yaw_correct_deg;
  }
  if(msg->ndt_y_correct >= -autoware_msgs::LocalizerCorrect::DISTANCE_CORRECT_TH &&
    msg->ndt_y_correct <= autoware_msgs::LocalizerCorrect::DISTANCE_CORRECT_TH)
  {
    position_y_correct = msg->ndt_y_correct;
  }
  math_tf_btol_corr();
  correctPublish();
}
...
static void param_callback(const autoware_config_msgs::ConfigNDT::ConstPtr& input)
{
  ...
  if(input->yaw_correct_deg >= -autoware_msgs::LocalizerCorrect::ANGLE_CORRECT_DEG_TH &&
    input->yaw_correct_deg <= autoware_msgs::LocalizerCorrect::ANGLE_CORRECT_DEG_TH)
  {
    yaw_correct_deg = input->yaw_correct_deg;
  }
  if(input->position_y_correct >= -autoware_msgs::LocalizerCorrect::DISTANCE_CORRECT_TH &&
    input->position_y_correct <= autoware_msgs::LocalizerCorrect::DISTANCE_CORRECT_TH)
  {
    position_y_correct = input->position_y_correct;
  }
  math_tf_btol_corr();
  correctPublish();
  /*Eigen::Translation3f tl_btol(_tf_x, _tf_y + input->position_y_correct, _tf_z);                 // tl: translation
  Eigen::AngleAxisf rot_x_btol(_tf_roll, Eigen::Vector3f::UnitX());  // rot: rotation
  Eigen::AngleAxisf rot_y_btol(_tf_pitch, Eigen::Vector3f::UnitY());
  Eigen::AngleAxisf rot_z_btol(_tf_yaw + input->yaw_correct_deg*M_PI/180.0, Eigen::Vector3f::UnitZ());
  tf_btol_corr = (tl_btol * rot_z_btol * rot_y_btol * rot_x_btol).matrix();*/
}
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    tf::Quaternion predict_q, ndt_q, ndt_q_corr, current_q, localizer_q, predict_ndt_q;
    ...
    Eigen::Matrix4f t2_corr(Eigen::Matrix4f::Identity());
    ...
    if (use_predict_pose == 0)
    {
      ...
      current_pose_corr.x = ndt_pose_corr.x;
      current_pose_corr.y = ndt_pose_corr.y;
      current_pose_corr.z = ndt_pose_corr.z;
      current_pose_corr.roll = ndt_pose_corr.roll;
      current_pose_corr.pitch = ndt_pose_corr.pitch;
      current_pose_corr.yaw = ndt_pose_corr.yaw;
    }
    else
    {
    ...
    t2_corr = t * tf_btol_corr.inverse();
    tf::Matrix3x3 mat_b_corr;  // base_link
    mat_b_corr.setValue(static_cast<double>(t2_corr(0, 0)), static_cast<double>(t2_corr(0, 1)), static_cast<double>(t2_corr(0, 2)),
                   static_cast<double>(t2_corr(1, 0)), static_cast<double>(t2_corr(1, 1)), static_cast<double>(t2_corr(1, 2)),
                   static_cast<double>(t2_corr(2, 0)), static_cast<double>(t2_corr(2, 1)), static_cast<double>(t2_corr(2, 2)));

    // Update ndt_pose
    ndt_pose_corr.x = t2_corr(0, 3);
    ndt_pose_corr.y = t2_corr(1, 3);
    ndt_pose_corr.z = t2_corr(2, 3);
    mat_b_corr.getRPY(ndt_pose_corr.roll, ndt_pose_corr.pitch, ndt_pose_corr.yaw, 1);

    ndt_q_corr.setRPY(ndt_pose_corr.roll, ndt_pose_corr.pitch, ndt_pose_corr.yaw);
    if (_use_local_transform == true)
    {
      tf::Vector3 v(ndt_pose_corr.x, ndt_pose_corr.y, ndt_pose_corr.z);
      tf::Transform transform(ndt_q_corr, v);
    ...
    else
    {
      ndt_pose_msg.header.frame_id = "/map";
      ndt_pose_msg.header.stamp = current_scan_time;
      ndt_pose_msg.pose.position.x = ndt_pose_corr.x;
      ndt_pose_msg.pose.position.y = ndt_pose_corr.y;
      ndt_pose_msg.pose.position.z = ndt_pose_corr.z;
      ndt_pose_msg.pose.orientation.x = ndt_q_corr.x();
      ndt_pose_msg.pose.orientation.y = ndt_q_corr.y();
      ndt_pose_msg.pose.orientation.z = ndt_q_corr.z();
      ndt_pose_msg.pose.orientation.w = ndt_q_corr.w();
    }

    current_q.setRPY(current_pose_corr.roll, current_pose_corr.pitch, current_pose_corr.yaw);
    ...
    */
    ...
    transform.setOrigin(tf::Vector3(current_pose_corr.x, current_pose_corr.y, current_pose_corr.z));
...
int main(int argc, char** argv)
{
  ...
  pub_correct = nh.advertise<autoware_msgs::LocalizerCorrect>("/ndt_localizer_correct", 10);
  ...
  ros::Subscriber way_param_sub = nh.subscribe("/waypoint_param", _queue_size * 10, way_param_callback);
  ros::Subscriber localizer_correct_sub = nh.subscribe("/smooth_transition_correct_ndt", _queue_size * 10, localizer_correct_callback);
  pthread_t thread;
