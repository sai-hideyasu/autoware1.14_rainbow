...
class Yolo3DetectorNode {
    ...
    ros::Publisher                  publisher_traffic_light_, publisher_traffic_light_pedestrian_;
    ...
    double                          image_ratio_h_;//resize ratio used to fit input image to network input size
    ...
    int                             detection_area_threshold_;
    std::vector<cv::Scalar>         colors_;

    std::vector<std::string>        custom_names_;
    bool                            use_coco_names_;

    bool                            yellow_red_flag_;//黃色での停止判定になるとtrue その後に黃色での進行判定になっても停止判定になる

    int                             judge_traffic_pedestrial_light(std::vector< RectClassScore<float> >& in_objects);
    int                             judge_traffic_light(std::vector< RectClassScore<float> >& in_objects);
    void                            pub_traffic_light(std::vector< RectClassScore<float> >& in_objects);
    std::vector<int>                detected_light_colors_, detected_pedestrian_light_colors_;
