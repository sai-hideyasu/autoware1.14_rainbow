...
void Nmea2TFPoseNode::initForROS()
{
  ...
  pub_direction_ = nh_.advertise<autoware_msgs::GnssDirection>("gnss_direction", 10);
  ...
  pub_direction_ = nh_.advertise<autoware_msgs::GnssDirection>("gnss_direction", 10);
  pub_time_count_ = nh_.advertise<autoware_msgs::GnssTimeDiffCount>("gnss_time_diff_count", 10);
  pub_gnss_sats_ = nh_.advertise<autoware_msgs::GnssSatsList>("gnss_sats_list", 10);
  pub_imu_status_ = nh_.advertise<autoware_msgs::ImuStatus>("gnss_imu_status", 10);
  pub_rtk_dop_ = nh_.advertise<autoware_msgs::RtkDop>("gnss_rtk_dop", 10);
}
  ...
  try
  {
    if(nmea.at(0) == "#BESTPOSA")
    {
      if(nmea.size() == 30)
      {
        ...
        autoware_msgs::GnssDirection direction;
        direction.header.frame_id = "gps";
        direction.header.stamp = current_stamp;
        direction.nmea_latitude = lat;
        direction.nmea_longitude = lon;
        direction.nmea_height = h;
        ...
        direction.fix_latitude_rad = geo_.get_m_lat();
        direction.fix_longitude_rad = geo_.get_m_lon();
        direction.roll_rad = roll_;
        direction.pitch_rad = pitch_;
        direction.azimuth_rad = yaw_;
        pub_direction_.publish(direction);
    ...
    else if(nmea.at(0) == "#RAWIMUA")
    {
      if(nmea.size() == 18)
      {
        ...
        uint32_t status_val = (uint32_t)std::stoi(nmea[11], nullptr, 16);
        autoware_msgs::ImuStatus imu_status;
        imu_status.header.stamp = current_stamp;
        std::stringstream ss_status;
        ss_status << std::bitset<16>(status_val);
        imu_status.status_bit = ss_status.str();
        imu_status.isa_100c_maintenance_indication      = status_val & 0x00000001;
        imu_status.isa_100c_accelerometers_invalid      = status_val & 0x00000002;
        imu_status.isa_100c_accelerometer_X_warning     = status_val & 0x00000004;
        imu_status.isa_100c_accelerometer_Y_warning     = status_val & 0x00000008;
        imu_status.isa_100c_accelerometer_Z_warning     = status_val & 0x00000010;
        imu_status.isa_100c_accelerometer_X_NOGO        = status_val & 0x00000020;
        imu_status.isa_100c_accelerometer_Y_NOGO        = status_val & 0x00000040;
        imu_status.isa_100c_accelerometer_Z_NOGO        = status_val & 0x00000080;
        imu_status.isa_100c_reset_occurred              = status_val & 0x00000100;
        imu_status.isa_100c_gyroscopes_invalid          = status_val & 0x00000200;
        imu_status.isa_100c_gyroscope_X_warning         = status_val & 0x00000400;
        imu_status.isa_100c_gyroscope_Y_warning         = status_val & 0x00000800;
        imu_status.isa_100c_gyroscope_Z_warning         = status_val & 0x00001000;
        imu_status.isa_100c_gyroscope_X_NOGO            = status_val & 0x00002000;
        imu_status.isa_100c_gyroscope_Y_NOGO            = status_val & 0x00004000;
        imu_status.isa_100c_gyroscope_Z_NOGO            = status_val & 0x00008000;
        uint32_t temperature = status_val >> 16;
        imu_status.isa_100c_temperature = temperature * 3.90625E-3;
    ...
    else if(nmea.at(0) == "#TIMEA")
    {
      if(nmea.size() == 20)
      {
        ...
        struct tm tmdate = {date.sec, date.min, date.hour, date.day, date.month, date.year-1900};
        time_t long_time = mktime(&tmdate);
        double diff_time = (long_time - time_a_) / (double)1000.0;
        time_a_ = long_time;
        count_bestpose_ = 0;
        count_inspvax_ = 0;
        count_insstdev_ = 0;
        ...
        autoware_msgs::GnssTimeDiffCount msg_count;
        msg_count.header.stamp = current_stamp;
        msg_count.count_bestpos = count_bestpose_;
        msg_count.count_inspvax = count_inspvax_;
        msg_count.count_insstdev = count_insstdev_;
        msg_count.count_rawimu = count_rawimu_;
        pub_time_count_.publish(msg_count);
      }
    }
    else if(nmea.at(0) == "#BESTSATSA")
    {
      autoware_msgs::GnssSatsList list;
      list.header.stamp = current_stamp;
      //list.header.frame_id = GPS_FRAME_;
      if(nmea.size() < 9)
      {
        pub_gnss_sats_.publish(list);
      }
      else
      {
        std::vector<std::string> nmea9 = split(nmea[9],';');
        if(nmea9.size() != 2)
        {
          pub_gnss_sats_.publish(list);
        }
        unsigned int count = std::atoi(split(nmea[9],';')[1].c_str());
        nmea.erase(nmea.begin(), nmea.begin() + 10);
        //std::cout << "nnn:" << nmea[0] << std::endl;
        if(count == nmea.size() / 4)
        {
          for(int i=0; i<nmea.size(); i+=4)
          {
            autoware_msgs::GnssSats sats;
            sats.system = nmea[i+0];
            sats.satellite_id = std::atoi(nmea[i+1].c_str());
            sats.status = nmea[i+2];
            sats.signal_mask = std::atoi(nmea[i+3].c_str());
            list.sats_list.push_back(sats);
          }
        }
        pub_gnss_sats_.publish(list);
      }
    }
    else if(nmea.at(0) == "#RTKDOP2A")
    {
