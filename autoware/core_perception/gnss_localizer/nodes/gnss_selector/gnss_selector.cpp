...
private:
	...
	ros::Publisher pub_gnss_stat_, pub_gnss_imu_, pub_gnss_time_, pub_gnss_sats_list_, pub_gnss_imu_status_;
	ros::Publisher pub_gnss_direction_;

	ros::Subscriber sub_gnss_pose_, sub_gnss_surface_speed_, sub_gnss_deviation_;
	ros::Subscriber sub_gnss_stat_, sub_gnss_imu_, sub_gnss_time_, sub_gnss_sats_list_, sub_gnss_imu_status_;
	ros::Subscriber sub_gnss_direction_;

	unsigned int publish_flag_;

	void CallbackGnssDirection(const autoware_msgs::GnssDirection &msg)
	{
		if(publish_flag_) pub_gnss_direction_.publish(msg);
	}
	...
	void CallbackGnssSatsList(const autoware_msgs::GnssSatsList &msg)
	{
		if(publish_flag_) pub_gnss_sats_list_.publish(msg);
	}

	void CallbackGnssImuStatus(const autoware_msgs::ImuStatus &msg)
	{
		if(publish_flag_) pub_gnss_imu_status_.publish(msg);
	}
public:
	GnssTopicList(ros::NodeHandle nh, ros::NodeHandle p_nh, std::string name_space, unsigned int class_id)
		...
		, publish_flag_(false)
	{
		std::stringstream gnss_direction_topic;
		gnss_direction_topic << name_space << "/gnss_direction";
		sub_gnss_direction_ = nh_.subscribe(gnss_direction_topic.str(), 1, &GnssTopicList::CallbackGnssDirection, this);
		pub_gnss_direction_ = nh_.advertise<autoware_msgs::GnssDirection>("/gnss_direction", 1);
		...
		std::stringstream gnss_sats_list;
		gnss_sats_list << name_space << "/gnss_sats_list";
		sub_gnss_sats_list_ = nh_.subscribe(gnss_sats_list.str(), 1, &GnssTopicList::CallbackGnssSatsList, this);
		pub_gnss_sats_list_ = nh_.advertise<autoware_msgs::GnssSatsList>("/gnss_sats_list", 1);

		std::stringstream gnss_imu_status_topic;
		gnss_imu_status_topic << name_space << "/gnss_imu_status";
		sub_gnss_imu_status_ = nh_.subscribe(gnss_imu_status_topic.str(), 1, &GnssTopicList::CallbackGnssImuStatus, this);
		pub_gnss_imu_status_ = nh_.advertise<autoware_msgs::ImuStatus>("/gnss_imu_status", 1);
	}
