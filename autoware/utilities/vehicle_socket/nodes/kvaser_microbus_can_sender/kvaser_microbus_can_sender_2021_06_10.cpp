...
private:
	...
	ros::Publisher pub_localizer_match_stat_, pub_stroke_routine_, pub_vehicle_status_, pub_velocity_param_, pub_tmp_, pub_acc_;
	...
	std::string vehicle_model;	//!< 車両モデル(リエッセ、中型、水陸両用車)
		...
		if(flag == true)
		{
			if(!ofs_log_writer_.is_open())
			{
				...
				if(sv.size() == 0)
				{
					...
					str << log_folder_ << "/NO_WAPOINT_FILE"  << "_" << rostime2date(ros::Time::now()) << ".csv";
				}
				else
				{
					str << log_folder_ << "/" << sv[sv.size()-1] << "_" << rostime2date(ros::Time::now()) << ".csv";
				}
	...
	void callbackMicrobusCan501(const autoware_can_msgs::MicroBusCan501::ConstPtr &msg)
	{
		...
		if(auto_log_write_ == true)
			logfileOpenOrClose(msg->auto_log);
	...
	void writeLog(ros::Time nowtime)
	{
		...
		//str << "," << can_receive_501_.stroke_cmd_reply;
		//name << "," << "can_receive_501_.stroke_cmd_reply";
		...
		//str << "," << can_receive_503_.cmd_voltage;
		//name << "," << "can_receive_503_.cmd_voltage";
		str << "," << shhv_.mechanism_steering_angle_main;
		name << "," << "shhv_.mechanism_steering_angle_main";
		str << "," << shhv_.mechanism_angle_sub;
		...
		//str << "," << can_receive_501_.steering_cmd_reply;
		//name << "," << "can_receive_501_.steering_cmd_reply";
...
//P
	...
	//!< okanuma
	std::vector<double> list_acceleration_accel;	//!< 加速度のリスト(section_cntの個数がサイズ)
	double past_velocity_accel = 0.0;	//!< 前回速度
	double past_sec_accel = 0.0;	//!< 前回取得時の時間
//	const double distance_margin = 1.0;	//!< stopDとの距離の乖離の余裕
	const double acceleration_margin_accel = 0.1;	//!< 理想加速度との乖離の余裕
	const double accel_i_min = 0.12;	//!< brake_iの最小値(appで変更できるようにしたほうが良い)
	//const double accel_i_max = 0.30;	//!< brake_iの最大値(appで変更できるようにしたほうが良い)
	double accel_i_max = 0.2;	//!< brake_iの最大値(appで変更できるようにしたほうが良い)
	const int section_cnt_accel = 10;//5	//!< 移動平均区間
	//!< ここまで	okanuma
	double _accel_stroke_pid_control(double current_velocity, double cmd_velocity, std::string &routine_str)
	{
		...
		if (pid_params.get_acclel_diff_sum_velocity() > setting_.accel_max_i)//この判定はまだ考慮の余地あり
		{
			...
			else if(vel_diff < 15) max_i = setting_.accel_max_i * 1.2;
			else max_i = setting_.accel_max_i * 1.3;
			e_i = max_i;
		}
		else
		{
...
//					routine_str = "accel5";
					...
					double acc = ((current_velocity/3.6 - past_velocity_accel) / (velocity_stamp - past_sec_accel));
					list_acceleration_accel.push_back(acc);
					//!< 移動平均リストをsection_cnt分にする
					
					while(list_acceleration_accel.size() > section_cnt)
					{
						list_acceleration_accel.erase(list_acceleration_accel.begin());
					}
					//!< section_cnt個数溜まったら計算を行う
					if(list_acceleration_accel.size() == section_cnt)
					{
						std::string acc_str;
						for(int i = 0; i < section_cnt; i++)
						{
							acc_str += std::to_string(list_acceleration_accel[i]);
							acc_str += ",";
						}

						//!< 加速度平均値計算
						double acceleration_ave = 0.0;
						for(double i : list_acceleration_accel)
						{
							acceleration_ave += i;
						}
						acceleration_ave /= (double)list_acceleration_accel.size();

						double ideal_acceleration = 1.9;	//!< 理想加速度計算
	...
	*/					
						{
							//!< 踏みまし量決定
							accel_i += (ideal_acceleration - acceleration_ave) * 1.0;
							if((waypoint_param_.global_twist.linear.x * 3.6) >= 50)
							{
								accel_i_max = 0.20;
							}else if((waypoint_param_.global_twist.linear.x * 3.6) >= 30)
							{
								accel_i_max = 0.18;
							}else if((waypoint_param_.global_twist.linear.x * 3.6) >= 20)
							{
								accel_i_max = 0.16;
							}else
							{
							...
							pub_list_acceleration.publish(std::to_string(accel_i));	//!< デバッグ用
							if(accel_i > accel_i_max) accel_i = accel_i_max;
							if(accel_i < accel_i_min) accel_i = accel_i_min;
						}
						//list_acceleration_accel.clear();
	...
	*/					
			...
			past_velocity_accel = current_velocity / 3.6;	//!<速度保管
			past_sec_accel = velocity_stamp;	//!< 現時刻保存
		}
		/*
		else
		{
			accel_i = setting_.k_accel_i_velocity;
			list_acceleration_accel.clear();
			past_velocity_accel = 0;	//!<速度保管
			past_sec_accel = 0;	//!< 現時刻保存
		}*/
		//!< ここまで okanuma
	...
	std::vector<double> list_acceleration;	//!< 加速度のリスト(section_cntの個数がサイズ)
	double past_velocity = 0.0;	//!< 前回速度
	double past_sec = 0.0;	//!< 前回取得時の時間
//	const double distance_margin = 1.0;	//!< stopDとの距離の乖離の余裕
	...
	//!< ここまで	okanuma
	double _brake_stroke_pid_control(double current_velocity, double cmd_velocity, double acceleration, std::string &routine_str)
	{
		routine_str = "brake";
		...
		if(pid_params.get_stroke_prev() > 0 && (stopper_distance_.distance < 0 || stopper_distance_.distance > stopper_distance_th))
		{
			...
			if(use_slow_accel_release_ == true && current_velocity > 0.1)//速度が０に近い場合は、加減処理をしない
			{
				if(stopD_first_velocity_ == 0) stopD_first_velocity_ = current_velocity;
				double stroke_kagen = math_stroke_kagen_brake(current_velocity);
		...
		if(stopper_distance_.distance > 0)
		{
			...
			if(past_sec != velocity_stamp)
			{
				...
				if(past_velocity != 0.0)
				{
					...
					//if(gnss_time_.sec < past_sec) list_acceleration.push_back((current_velocity/3.6 - past_velocity) / (60 - past_sec + gnss_time_.sec));
					//else list_acceleration.push_back((current_velocity/3.6 - past_velocity) / (gnss_time_.sec - past_sec));
					double acc = ((current_velocity/3.6 - past_velocity) / (velocity_stamp - past_sec));
					list_acceleration.push_back(acc);
					//!< 移動平均リストをsection_cnt分にする
					...
					if(list_acceleration.size() == section_cnt)
					{
						...
						double ideal_acceleration = -10;	//!< 初期値はどうするか不明
						if(stopper_distance_.distance > setting_.stopper_distance3)
						{
							ideal_acceleration = (pow(stopper_distance_.fixed_velocity, 2)-pow((current_velocity/3.6), 2)) / (2 * stopper_distance_.distance  - setting_.stopper_distance3);	//!< 理想加速度計算
						}else{
							//!< distance3以内の状態の理想加速度をどうするか
						}
	...
	*/					
			...
			past_velocity = current_velocity / 3.6;	//!<速度保管
			past_sec = velocity_stamp;	//!< 現時刻保存
		}
		else
		{
			routine_str = "brake7";
											 ...
											 stopper_distance_.send_process == autoware_msgs::StopperDistance::OBSTACLE))
		{
			...
			else if(stopper_distance_.distance >= 0 && stopper_distance_.distance <= setting_.stopper_distance3)
			{
				...
				if(stopper_distance_.fixed_velocity == 0 || stopper_distance_.send_process == autoware_msgs::StopperDistance::SIGNAL)
				{
					...
					if(target_brake_stroke < 150) target_brake_stroke = 150;
...
pub_tmp_.publish(str_ret);*/
	...
	void bufset_car_control(unsigned char *buf, double current_velocity)
	{
		...
		//buf[7] |= 0x20;
		if((err_distance_ || err_localizer_time_ || err_localizer_stat_ || err_gnss_dev_) && use_error_check_)
		{
			buf[7] |= 0x20;
			buf[7] |= 0x10;
			//buf[6] |= 0x08;
